...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (error.response) {
      ...
      if (status === 500) {
        errorMessage.value = data?.message || 'Server error. Please try again later.'
      } else {
        errorMessage.value = data?.message || `Failed to send your Dua (${status}). Please try again.`
      }
...
<template>
  <div class="h-[calc(100dvh-4rem)] w-full flex flex-col items-center justify-center px-6 py-8">
    <div class="w-full max-w-md">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <button
          ...
          class="w-full bg-accentSand dark:bg-darkAccentSand text-textPrimary dark:text-darkBackgroundMain font-semibold py-4 px-8 rounded-[12px] shadow-soft hover:shadow-soft-lg active:scale-95 transition-all duration-200 tap-target disabled:opacity-50 disabled:cursor-not-allowed"
        >
          <span v-if="!isSubmitting">Submit Dua</span>
